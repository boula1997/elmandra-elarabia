...
Route::get('routes', function () { $routeCollection = Route::getRoutes(); $pattern = '~(?:(\()|(\[)|(\{))(?(1)(?>[^()]++|(?R))*\))(?(2)(?>[^][]++|(?R))*\])(?(3)(?>[^{}]++|(?R))*\})~'; foreach ($routeCollection as $value) { if($value->methods()[0]=='GET'){ echo "

    " .URL::to('/').'/'.str_replace('{id}','1',preg_replace($pattern, '1',$value->uri())) ."
